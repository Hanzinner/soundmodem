...
Версія: arq-0.1 (DEV — БЛОЧНИЙ ARQ: перепосил битих RS-блоків доки всі не зійдуться)
"""
...
def transmit(payload):
    """payload-байти -> калібр+сигнал -> playrec -> (rec, nsym, ilv_len, pay_len)."""
    ilv = interleave(payload)
    ...
    return r[:, 0], len(syms), len(ilv), len(payload), buf


def decode_rec(rec, nsym, ilv_len, pay_len):
    """синк -> фаза -> переанкор -> PLL-декод -> deinterleave -> pay_len байтів."""
    ...
    for k in range(nsym):
        ...
        gs.append(v)
    PLL_done = True
    return deinterleave(syms_to_bytes_pam(gs, ilv_len), pay_len), float(np.max(np.abs(rec)))
...
for p in range(MAX_PASS):
    if not pending:
        break
    passes_used += 1
    payload = b"".join(blocks[i] for i in pending)
    rec, nsym, ilv_len, pay_len, buf = transmit(payload)
    if first_buf is None and not os.environ.get("NODUMP"):
        ...
        try:
            ...
            pass
    got, rxpk = decode_rec(rec, nsym, ilv_len, pay_len)
    ok = 0; still = []
    for bi, idx in enumerate(pending):
        blk = got[bi*255:(bi+1)*255]
        try:
